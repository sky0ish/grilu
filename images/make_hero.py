...
x0 = W - pw
fade = 70
alpha = np.ones((ph, pw, 1), np.float32)
alpha[:, :fade, 0] = np.linspace(0, 1, fade) ** 1.4
canvas[:, x0:, :] = canvas[:, x0:, :] * (1 - alpha) + px * alpha

# 4) 지면(어두운 언덕)을 왼쪽까지 이어서 사람들 발밑 선이 끊기지 않게
ground_row = px[-1, :fade, :].mean(axis=0)              # 사진 맨 아래 왼쪽 색
for y in range(H - 70, H):
    a = ((y - (H - 70)) / 70) ** 2
    canvas[y, :x0 + fade, :] = canvas[y, :x0 + fade, :] * (1 - a) + ground_row * a
